...
final class Filer_Delegate {
    init(modes:[Mode] = [.cases, .contacts, .folders], actions:[Action] = Action.allCases) {
        self.modes = modes
    ...
    enum Mode : Equatable, Hashable {
        case cases, folders, aCase(Case), contacts, aContact(Contact)
        var title : String {
            switch self {
            case .cases:
                "Cases"
    ...
    var isInSingleCaseMode : Bool {
        ...
        case .cases, .folders, .contacts, .aContact(_):
            false
        case .aCase(_):
            true
        }
    ...
    enum Action : String, CaseIterable {
        case newCase, newContact, newDrive, newFolder, filterCases, filterContacts, filterDrive, filterFolders, cancel, reset, saveTo, trash
        static var inlineActions : [Action]  { [.newCase, .newContact, .newDrive, .newFolder, .filterCases, .filterContacts, .filterDrive, .saveTo, .reset, .trash  ]}
        static var sheetActions  : [Action]  { [.newCase, .newContact, .newDrive, .newFolder, .filterCases, .filterContacts, .filterDrive, .saveTo, .cancel, .trash  ]}
        static var altSheetActs  : [Action]  { [.newCase, .newContact, .newDrive, .newFolder, .filterCases, .filterContacts, .filterDrive, .cancel , .trash          ]} //no saveTo
    }
    var actions : [Action]
    ...
    //Contacts
    private(set) var centralContacts        : [Contact] = []
    private(set) var selectedContact        : Contact?
                 var contactListSelection   : Contact?
    private(set) var contactListScrollID    : Contact.ID?
    ...
    enum FilingState : Equatable{
        ...
        case idle, caseSelected(Case), contactSelected(Contact), folderSelected(GTLRDrive_File), formPresented(Case?, GTLRDrive_File?), isFiling, filed([Filer_Item], Bool), caseUpdated(Case), contactUpdated(Contact), error(Error)
        var intValue : Int {
            switch self {
            case .idle:
                0
            case .caseSelected(_):
                1
            case .contactSelected(_):
                2
            case .folderSelected(_):
                3
            case .formPresented(_,_):
                4
            case .isFiling:
                5
            case .filed(_, _):
        ...
        var title : String {
            ...
            case .contactSelected(let contact):
                "Selected Contact: \(contact.label.name)"
            case .contactUpdated(let contact):
                "Updated Contact: \(contact.label.name)"
...
extension Filer_Delegate {
    var canFileItems : Bool {
        ...
        let canFileToMode = switch selectedMode {
        case .cases, .aCase(_):
            true
        case .folders:
            true
        ...
        guard canFileToMode else { return false }
...
//MARK: - Contacts {
extension Filer_Delegate {
    func resetContactVariables() {
        centralContacts      = []
        selectedContact      = nil
        contactListSelection = nil
        contactListScrollID  = nil
        filterString      = ""
        resetFormData()
    }
    func select(_ contact:Contact)  {
        filingState = .contactSelected(contact)
        selectedContact = contact
        let folderID = contact.file.parents?.first ?? "We have a problem"
        let folder = GTLRDrive_File()
        folder.identifier = folderID
        folder.name = contact.label.name
        folder.mimeType = GTLRDrive_File.MimeType.folder.rawValue
        //loader.forceIsLoading()
        stack = [folder]
        Task { await loadFolders() }
    }
    func loadContacts() async {
        do {
            ...
            centralContacts = try await Drive.shared.get(filesWithLabelID:Contact.DriveLabel.id.rawValue)
...
//        print("New Tasks: \(newTasks)")
    ...
    //Update Contact Spreadsheet
    fileprivate func updateSpreadsheet(for contact:Contact) async throws {
        do {
            loader.start("Updating \(contact.label.name) Sheet")
            let newFiles : [Contact.File] = items.compactMap { item in
                guard let file = item.file else { return nil }
                return Contact.File(id: UUID().uuidString,
                                    dateID: Date.idString,
                                    fileID: file.id,
                                    category: auxString,
            ...
            try await Sheets.shared.append(newFiles, to: contact.id)
            filingState = .contactUpdated(contact)
            loader.stop()
        } catch {
            loader.stop(error)
            throw error
    ...
    func saveToSelectedContact() async throws {
        do {
            guard let contact  = selectedContact else { throw NSError.quick("No Contact Selected"  ) }
            guard let folderID = contact.file.parents?.first else { throw NSError.quick("No Contact Folder ID Found")}
            loader.start("Getting Contact Folder")
            filingState = .isFiling
            let folder   = try await Drive.shared.get(folder:auxString, parentID:folderID, createIfNotFound: true, caseInsensitive: true)
            //Save the items to the appropriate directory
            try await saveTo(folder.id)
            
            //Update the Case Spreadsheet
            try await updateSpreadsheet(for: contact)
            loader.stop()
        } catch {
            loader.stop()
            throw error
