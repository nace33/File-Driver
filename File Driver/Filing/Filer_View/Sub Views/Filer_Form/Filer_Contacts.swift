...
struct Filer_Contacts: View {
    ...
    var body: some View {
        ...
        Section {
            FormTokensPicker2(title: "Contacts", items: Bindable(delegate).contacts, allItems:eligible, titleKey: \.name, altColor:.orange, create:  { createString in
               addNewFilingContact(createString)
            })
            .onChange(of: delegate.contacts) {
                updatedContacts(oldValue: $0, newValue: $1)
            }
        }
...
//MARK: - Load Conctacts from Filer Items
...
//                print("Make new Contact")
    ...
    //Change in TokenPicker
    func updatedContacts(oldValue:[Case.Contact], newValue:[Case.Contact]) {
        ///remove new contactsData for any Contact that was removed from the token picker
        ///this is normally when an email address was found in loadFilerItemContacts that is new to the case, but the user then removes the contact in the tokenPicker UI
        ///This does not need to be done in Filer_Tags because no data (other htan the tag) is added or removed.
        let removed  = Set(oldValue).subtracting(Set(newValue))
        let removedIDs = removed.map(\.id)
        delegate.contactData.removeAll(where: {removedIDs.contains($0.contactID)})
    }
}
