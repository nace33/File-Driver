...
struct EditTemplate: View {
    ...
    @State private var status = "Edit Template"
    ...
    var body: some View {
        EditForm(title: status, prompt: "Update", item: $template) { editItem in
            ...
            if editItem.wrappedValue.file.title != editItem.wrappedValue.label.filename {
                try await controller.rename(template: editItem, newFilename: editItem.wrappedValue.label.filename ) { status = $0 }
            }
            if editItem.wrappedValue.label != originalLabel {
                try await controller.updateDriveLabel(editItem) { status = $0 }
            }
            dismiss()
        }
    }
