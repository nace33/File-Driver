...
@Observable
final class TemplatesDelegate {
    static let shared: TemplatesDelegate = { TemplatesDelegate() }()
    let title = "Template"
...
extension TemplatesDelegate {
    ...
    func loadCategories() {
        categories = filteredTemplates.compactMap { $0.wrappedValue.label.category}
                              .unique()
                              .sorted { $0.ciCompare($1)}
        
        subCategories = filteredTemplates.compactMap { $0.wrappedValue.label.subCategory}
                                 .unique()
                                 .sorted { $0.ciCompare($1)}
    }
...
//    var selectedTemplates  : Binding<Template>? { filteredTemplates.first(where: {$0.wrappedValue.id == selectedRootID})}
    var filteredTemplates : Binding<[Template]> {
        Binding {
            let shows :[Template.Show] = UserDefaults.getEnums(forKey: BOF_Settings.Key.templatesShow.rawValue)
    ...
    func checkSelection()  {
        let currentIDs =  filteredTemplates.wrappedValue.map(\.id)
...
extension TemplatesDelegate {
    ...
    func create(_ template:Template, moveFile:GTLRDrive_File?,  duplicateFile:String?, progress:(String)->Void) async throws -> Template {
        do {
            ...
            if let moveFile {
                ...
                newFile = try await Drive.shared.move(file: moveFile, to: destinationFolder)
            }
            else if let duplicateFile {
...
//MARK: - Remove
extension TemplatesDelegate {
    fileprivate func remove(_ template:Template) {
        if selectedIDs.contains(template.id) {
            selectedIDs.removeAll()
        }
        templates.removeAll(where: {$0.id == template.id})
    }
    func removeLabel(_ template:Template) async throws {
        do {
//            loader.start("Removing Label")
      
            try await Drive.shared.lable(remove:Template.DriveLabel.id.rawValue, on:[template.file.id])
            remove(template)
//            loader.stop()
        } catch {
//            loader.stop(error)
    ...
    func trash(_ template:Template) async throws {
        do {
//            loader.start( "Moving to Trash")
            _ = try await Drive.shared.delete(ids: [template.file.id])
            remove(template)
//            loader.stop()
        } catch {
//            loader.stop(error)
