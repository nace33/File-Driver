...
public
struct FormTokensPicker2<T : Identifiable & Hashable> : View {
    private let title : String
    private let items: Binding<[T]>
    private let allItems : [T]
    private let titleKey  : KeyPath<T, String>
    ...
    public var body: some View {
        LabeledContent {
            VStack(alignment:.trailing, spacing:10) {
                if create != nil {
                    TextField_Suggestions(title, text: $text, prompt: Text("Add \(title)"), style: .both, suggestions: allItems.compactMap({$0[keyPath: titleKey]})) { _ in
                        submitted()
                    }
                        .multilineTextAlignment(.trailing)
                        .labelsHidden()
