...
extension Case {
    struct DriveLabel :Equatable  {
        
        var title      : String
        var category   : Label.Field.Category
        var status     : Label.Field.Status
        var opened     : Date
        var closed     : Date?
        var folderID   : String
        var audit      : Label.Field.Audit
        ...
        init?(driveLabel:GTLRDrive_Label?) {
            guard let driveLabel,
                  ...
                  let audit    = Label.Field.Audit(driveLabel)
            else { return nil }
            self.title    = title
            self.category = cat
            self.status   = stat
...
extension Case.DriveLabel {
    //MARK: -Properties
    var year : String {
        opened.formatted(Date.FormatStyle().year(.twoDigits))
    }
    var categoryShortTitle : String { category.abbreviation }
    var folderTitle : String {
       "[\(year)_\(categoryShortTitle)] \(title)"
    }
...
extension Case.DriveLabel {
    ...
    //Ex: _ = try await Drive.shared.labelModify(labelID: DriveLabel.Label.id.rawValue, modifications:[label.labelModification], on:fileToModify.id)
    fileprivate var labelFieldModifications : [GTLRDrive_LabelFieldModification] {
        var mods = [GTLRDrive_LabelFieldModification]()
        if let mod =  Drive.shared.label(modify: Label.Field.title.rawValue, value: title, valueType: .text) {
            mods.append(mod)
        }
        if let mod =  Drive.shared.label(modify: Label.Field.category.rawValue, value:category.rawValue, valueType: .selection) {
            mods.append(mod)
        }
        if let mod =  Drive.shared.label(modify: Label.Field.status.rawValue, value:status.rawValue, valueType: .selection) {
            mods.append(mod)
        }
        if let mod =  Drive.shared.label(modify: Label.Field.opened.rawValue, value: opened, valueType: .date) {
            mods.append(mod)
        }
        if let mod =  Drive.shared.label(modify: Label.Field.closed.rawValue, value: closed, valueType: .date) {
            mods.append(mod)
        }
        if let mod =  Drive.shared.label(modify: Label.Field.folderID.rawValue, value: folderID, valueType: .text) {
            mods.append(mod)
        }
        if let mod =  Drive.shared.label(modify: Label.Field.audit.rawValue, value:audit.rawValue, valueType: .selection) {
            mods.append(mod)
        }
        return mods
    }
    var labelModification       : GTLRDrive_LabelModification {
        Drive.shared.label(modify: Label.id.rawValue, fieldModifications: labelFieldModifications)
    }
}
...
extension Case.DriveLabel : CustomStringConvertible{
    ...
    var consultationHeader : String {
        "\(Calendar.current.component(.year, from: opened) % 100)_\(category.abbreviation)"
    }
...
extension Case.DriveLabel {
    enum Label : String, CaseIterable {
        ...
        enum Field : String, CaseIterable {
            ...
            enum Audit  : String, CaseIterable, Comparable {
                case no  = "B9BF2035FB"
                case yes = "A124403966"
                var intValue : Int {
                    Audit.allCases.firstIndex(of: self) ?? -1
                }
                var boolValue : Bool {  self == .yes  }
                static func < (lhs: Label.Field.Audit, rhs: Label.Field.Audit) -> Bool {
                    lhs.rawValue == rhs.rawValue
                }
                init?(_ driveLabel:GTLRDrive_Label) {
                    guard let val = driveLabel.field(id:Label.Field.audit.rawValue)?.selection?.first,
                          let a  = Audit.allCases.first(where: {$0.rawValue == val}) else { return  nil }
                    self = a
                }
            }
        }
