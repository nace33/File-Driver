...
extension Case_OLD {
    ...
    fileprivate func createDriveFolder(name:String, parentID:String) async throws(Case_Error) -> GTLRDrive_File {
        do {
           return try await Drive.shared.create(folder: name, in:parentID)
        } catch {
...
extension Case_OLD {
    ...
    fileprivate func update(_ folder:Folder, newName: String) async throws(Case_Error) {
        do {
          _ = try await Drive.shared.rename(id: folder.id, newName: newName)
...
extension Case_OLD {
    ...
    fileprivate func deleteFromDrive(_ folder:Folder) async throws(Case_Error) {
        do {
            guard try await Drive.shared.getContents(of:folder.id).isEmpty else {
                throw Case_Error.cannotDeleteFolderThatIsNotEmpty
            }
            _ = try await Drive.shared.delete(ids: [folder.id])
        } catch {
