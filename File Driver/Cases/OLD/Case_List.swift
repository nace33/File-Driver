...
struct Case_List : View {
    @State private var selected : Case_OLD?
    ...
    func loadCases() async {
        ...
        self.error = nil
        do {
            let caseSpreadsheets = try await Drive.shared.get(filesWithLabelID:caseLabelID)
            cases = caseSpreadsheets.compactMap { Case_OLD($0)}
