...
struct Cases_Table: View {
    @AppStorage(BOF_Settings.Key.casesFilingTableColumnKey.rawValue)     var columnKey  : TableColumnCustomization<Case_OLD>
    @State private var sortOrder = [KeyPathComparator(\Case_OLD.title)]
    @State private var cases: [Case_OLD] = []
    @State private var selectedID : Case_OLD.ID?
    @AppStorage(BOF_Settings.Key.caseFilingShowStatusColorsKey.rawValue)  var showStatusColors : Bool = true

    var filteredCases: [Case_OLD] {
        cases
    }
    var body: some View {
        tableView
    ...
    func tableSortKeyPaths() -> [String] {
        sortOrder
            .map {
                ...
                case \Case_OLD.title:
                    keyPathString = "Title"
                case \Case_OLD.driveLabel.category.title:
                    keyPathString = "Category"
                case \Case_OLD.driveLabel.status.intValue:
                    keyPathString = "Status"
                case \Case_OLD.driveLabel.opened:
                    keyPathString = "Opened"
                case \Case_OLD.driveLabel.closed:
