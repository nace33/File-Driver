...
extension NewCase {
    func create() async {
        do {
            ...
            _ = try await Sheets.shared.initialize(id: newCaseFile.id, headers: Case.Sheet.allCases.compactMap(\.headerRow))
...
//MARK: -View Builders
extension NewCase {
    @ViewBuilder var templateSelectorView : some View {
        DriveSelector("Select Case Template", canLoadFolders: true, mimeTypes: [.sheet]) { file in
            templateID = file.id
        }
    }
