...
struct Case_Contact_Details: View {
    ...
    init(aCase: Case, contact: Case.Contact) {
        self.aCase = aCase
        self.contact = contact
    }
    @State private var details : [Case.Contact] = []
    ...
    var categorySections : [Case.Contact.Category] { [
        .role, .credential, .email, .phone, .address, .fax, .social, .website, .date, .number, .other, .note
    ]}

    func details(category:Case.Contact.Category) -> [Case.Contact] {
